...
PACKAGES = zsh git tmux tool-versions nvim claude direnv ssh local-bin ghostty pi tig opencode herdr corne starship
...
deps: ## Check required dependencies
	@ok=true; \
	for cmd in brew stow git nvim tmux asdf mise tree-sitter direnv opam jq curl cargo claude elan pipx rg gcc unzip node stylua reattach-to-user-namespace qmd fswatch pi tig gh glow agent-browser gopls go; do \
